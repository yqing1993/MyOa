...
//瘦脸词录入
class InsertBrandAction extends loginAction{
    ...
    public function index(){
        ...
        $res = m('k_category')->where("CategoryName = '品牌词'")->field('id')->select();
        $id = $res[0]['id'];
        $where['CategoryID'] = $id;
        $where['status'] = '0';
        $Projects = m('k_project')->where($where)->order('concat(rank, addtime) desc')->field('id,ProjectName')->select();
        $this->assign('Projects', $Projects);
        $this->display();
    }

    public function InsertBrandPage(){
        $res = m('k_category')->where("CategoryName = '品牌词'")->field('id')->select();
        $id = $res[0]['id'];
        $userid = $_SESSION['userid'];
        $data = DataPage(array('database'=>'keywords','order'=>'concat(rank, addtime) desc','para'=>" and userid = '$userid' and CategoryID = '$id'"));
        ...
        if($id != ''){
            ...
            if($operate == 'UpdateAdd'){
                ...
                if($id == 'new'){
                    ...
                    if($repeat == '0'){
                        $CategoryID = m('k_project')->where('id ='.$add['ProjectID'])->field('CategoryID')->find();

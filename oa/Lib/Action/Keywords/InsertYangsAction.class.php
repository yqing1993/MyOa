...
//瘦脸词录入
class InsertYangsAction extends loginAction{
    ...
    public function index(){
        ...
        $res = m('k_project')->where("ProjectName = '养生'")->field('id')->select();
        $id = $res[0]['id'];
        $where['ProjectID'] = $id;
        $where['status'] = '0';
...
//        $this->assign('data', $data);
    ...
    public function InsertYangsPage(){
        $res = m('k_project')->where("ProjectName = '养生'")->field('id')->select();
        $id = $res[0]['id'];
        $userid = $_SESSION['userid'];
    ...
    public function InsertYangsData(){
        $id = I('post.id');

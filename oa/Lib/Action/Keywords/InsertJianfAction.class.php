...
//瘦脸词录入
class InsertJianfAction extends loginAction{
    ...
    public function index(){
        ...
        $res = m('k_project')->where("ProjectName = '减肥'")->field('id')->select();
        $id = $res[0]['id'];
        $where['ProjectID'] = $id;
        $where['status'] = '0';
...
//        $this->assign('data', $data);
    ...
    public function InsertJianfPage(){
        $res = m('k_project')->where("ProjectName = '减肥'")->field('id')->select();
        $id = $res[0]['id'];
        $userid = $_SESSION['userid'];
    ...
    public function InsertJianfData(){
        $id = I('post.id');

...
//关键词类型处理 行业词/品牌词
class KeCategoryAction extends loginAction{
    ...
    public function KeCategoryPage(){
        $data = DataPage(array('database'=>'k_category','order'=>'rank desc'));
        echo json_encode($data);
    }

    public function KeCategoryData(){
        $id = I('post.id');
        ...
        if($id != ''){
            ...
            if($operate == 'UpdateAdd'){
                ...
                if($id == 'new'){
                    /*检测重复*/
                    $repeat = m($database)->where("CategoryName = '".$add['CategoryName']."'")->count();
                    ...
                    if($repeat > 0){
                        /*检测有没有重复*/
                        $repeat = m($database)->where("id != '".$id."' and CategoryName = '".$add['CategoryName']."'")->count();
                        if($repeat == '0'){

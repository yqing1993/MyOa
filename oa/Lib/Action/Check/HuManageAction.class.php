<?php
class HuManageAction extends loginAction {
	...
	public function HuManagePage(){
		$data = DataPage(array('database'=>'j_hu','order'=>'rank desc'));

		echo json_encode($data);
	}

	public function HuManageData(){
		$id = I('post.id');
		...
		if($id != ''){
			...
			if($operate == 'UpdateAdd'){
				...
				if($id == 'new'){
					/*检测重复*/
					$repeat = m($database)->where("HuName = '".$add['HuName']."'")->count();
					if($repeat == '0'){
					...
					if($repeat > 0){
						/*检测有没有重复*/
						$repeat = m($database)->where("id != '".$id."' and HuName = '".$add['HuName']."'")->count();
						if($repeat == '0'){

<?php
class ZhaoWebAction extends loginAction {
	...
	public function ZhaoWebPage(){
		$data = DataPage(array('database'=>'j_zhaoweb','order'=>'rank desc'));

		echo json_encode($data);
	}

	public function ZhaoWebData(){
		$id = I('post.id');
		...
		if($id != ''){
			...
			if($operate == 'UpdateAdd'){
				...
				if($id == 'new'){
					/*检测重复*/
					$repeat = m($database)->where("ZhaoWebName = '".$add['ZhaoWebName']."'")->count();
					if($repeat == '0'){
					...
					if($repeat > 0){
						/*检测有没有重复*/
						$repeat = m($database)->where("id != '".$id."' and ZhaoWebName = '".$add['ZhaoWebName']."'")->count();
						if($repeat == '0'){

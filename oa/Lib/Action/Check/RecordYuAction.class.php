<?php
class RecordYuAction extends loginAction {
	...
	public function RecordYuPage(){
		$data = DataPage(array('database'=>'j_recordyu','order'=>'Day desc,id desc'));

		echo json_encode($data);
	}

	public function RecordYuData(){
		$id = I('post.id');
		...
		if($id != ''){
			...
			if($operate == 'UpdateAdd'){
				...
				if($id == 'new'){
					...
					$repeat = array();
					foreach($add as $k=>$v){
						$count = m($database)->where("TuiWebID = '".$v['TuiWebID']."' and Day = '".$v['Day']."'")->field('addtime')->find();
